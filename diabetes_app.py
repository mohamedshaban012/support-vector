...
def main():
    ...
    try:
        ...
        if df.isnull().any().any():
            st.warning("Warning: The dataset contains missing values. Dropping rows with missing values.")
            df = df.dropna()
        ...
        # Verify data shapes
        if len(X) != len(y):
            st.error("Error: Number of samples in features and target don't match!")
            return
        ...
        st.write(f"Training samples: {X_train_scaled.shape[0]}, Target samples: {len(y_train)}")
